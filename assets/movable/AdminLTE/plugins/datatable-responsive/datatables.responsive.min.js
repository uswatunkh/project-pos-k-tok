...
ResponsiveDatatablesHelper.prototype.init = function (a, b) {
	...
	if (!this.disabled) {
		...
		if (!this.skipNextWindowsWidthChange){
			if (0 === this.lastBreakpoint.length && c.length){
				f = !0;
			...
			if (this.columnsHiddenIndexes.length !== c.length) {
				f = !0;
			}
			else {
	...
	for (var c = a.api, d = c.columns().header(), e = $(a.rowTemplate), f = $("ul", e), g = 0; g < a.columnsHiddenIndexes.length; g++) {
		...
		if (!a.options.hideEmptyColumnsInRowDetail || j.innerHTML.trim().length) {
			...
			var column_name;
			if(d[h].innerHTML=='<input value="" type="checkbox">'){
				column_name = 'checkboxs';
			}
			else{
				column_name = d[h].innerHTML;
			}
			//console.log(d[h].innerHTML);
			
			
			$(".columnValue", k).append(n).data("originalTdSource", j), k.attr("data-column", column_name);
