 ...
 ;(function($, undefined) {
	...
	$.fn.SuperBox = function(options) {
		...
		var sbIsIconShown = false,
			...
			sbImg = $('<img src="img/ajax-loader.gif" class="superbox-current-img"/><div class="col-sm-12 paman-info"><ul>	<li class="paman-name">name</li>	<li> Visible : <span class="paman-look" ></span></li>	<li><a><button  class="btn btn-xs btn-info ">Edit</button></a> &nbsp;	<button onclick="delFunction(this.id)" class="btn btn-xs btn-danger btn-del" data-toggle="modal" data-target="#myModal">Delete</button></li>	</ul></div>'),
		...
		var createSuperboxShow = function(elem){
			 ...
			 */
				...
				setImageData = function(){
					...
					sbWrapper.find('.paman-info button.btn-del').attr('id',elem.find('img').data('id'));
					sbWrapper.find('.paman-info button.btn-del').attr('class',elem.find('img').data('hidedel'));
					
					sbWrapper.find('.paman-info a').attr('href',elem.find('img').data('edit'));
					sbWrapper.find('.paman-info a').attr('class',elem.find('img').data('hidedit'));
					
					sbWrapper.find('.paman-look').html(elem.find('img').data('look'));
					sbWrapper.find('.paman-name').html(elem.find('img').data('label'));
					preloadImageData();
				},
