...
if($privilege['U']=="1" || $privilege['D']=="1"){
	...
	if($privilege['U']=="1"){
		$action .= '<li><a class="btn-edit" href="#">Edit</a></li>
					<li><a class="btn-top" href="#">Set as Slider</a></li>';
	}
...
$(function() {
	...
	var dt = $("#dt_basic").DataTable({
		...
		// { data: "blog.slider_priority"},
		{ data: "blog.judul" },
		{ data: "blog.file" },
		{ data: "blog.user_view" },
		{ data: "blog.tgl" },
		{ data: "wd_users.wd_users:username", orderable: false},
		// { data: "blog.tag" },			
		{
				data: null,
	...
	$("#dt_basic tbody").on("click", ".btn-top", function() {
		var tr = $(this).closest("tr");
		var recordId = tr.attr("id");
		window.location =  JS_BASE_URL + "<?php echo this_module();?>/on_top?id=" + recordId;
	});
